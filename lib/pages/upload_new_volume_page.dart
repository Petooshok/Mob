...
class _UploadNewVolumePageState extends State<UploadNewVolumePage> {
  ...
  // Список с готовыми текстами для формата, издателя и количества глав
  ...
  void _submit() {
    if (_validateInputs()) {
      final newMangaItem = MangaItem(
        ...
        additionalImages: _imageLinks.sublist(1), // Дополнительные изображения, исключая первое
        format: _formatController.text,
        publisher: _publisherController.text,
        shortDescription: _shortDescriptionController.text, // Добавляем краткое описание
        chapters: _chaptersController.text, // Добавляем главы
      );
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          onChanged: (String? newValue) {
            setState(() {
              if (newValue == 'Другое') {
                controller.clear(); // Очищаем поле для ввода
              } else {
  ...
  // Функция для создания текстовой области
